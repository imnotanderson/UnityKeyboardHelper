...
public class MyUIInput : MonoBehaviour
{
	...
	static public MyKeyboard mKeyboard;
    ...
    public void Set(string value, bool notify = true, bool fromAndroid = false)
	{
#if UNITY_EDITOR
		if (!Application.isPlaying) return;
#endif
	...
	protected virtual void Update ()
	{
		...
		if (mSelectMe != -1 && mSelectMe != Time.frameCount)
		{
			...
			if (true || pf == RuntimePlatform.IPhonePlayer
    ...
    public class MyKeyboard {
        ...
        public MyKeyboard() {
            try
            {
                UnityKeyboardHelper.Show(MyUIInput.current.value);
            }catch(Exception e)
            {
                Debug.Log(e);
            }
    ...
    public void OnSelect(int start, int end) {
        if (myStart != start || myEnd != end) {
            myStart = start;
            myEnd = end;
            UnityKeyboardHelper.SetSelect(start, end);
        }
    }
}
